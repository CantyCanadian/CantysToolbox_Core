...
namespace Canty.Core
{
    ...
    public class PositiveValueOnlyAttributeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            {
                EditorGUI.LabelField(position, new GUIContent("Error, non-numeric.", "[PositiveValueOnly] used with non-numeric property."));
            }
            else
            {
        ...
        private bool HandleIntVectors(SerializedProperty property)
        {
            if (property.propertyType == SerializedPropertyType.Vector2Int)
            {
                ...
                property.vector2IntValue = new Vector2Int(vector.x < 0 ? 0 : vector.x, vector.y < 0 ? 0 : vector.y);
                return true;
            }
            ...
            if (property.propertyType == SerializedPropertyType.Vector3Int)
            {
                ...
                property.vector3Value = new Vector3(vector.x < 0 ? 0 : vector.x, vector.y < 0 ? 0 : vector.y, vector.z < 0 ? 0 : vector.z);
                return true;
            }
